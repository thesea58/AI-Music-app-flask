...
class MusicVocab:
    "chứa sự tương ứng giữa các numbers và tokens và numericalize."
    ...
    def numericalize(self, t: Collection[str]):
        "Convert a list of tokens `t` to their ids."
        "Chuyển đổi danh sách mã thông báo `t` thành id "
        return [self.stoi[w] for w in t]

    def textify(self, nums: Collection[int], sep=' '):
        "Chuyển đổi danh sách `nums` thành `tokens`"
        items = [self.itos[i] for i in nums]
        return sep.join(items) if sep is not None else items

    @classmethod
    def create(cls):
        "Tạo từ vựng từ một bộ `tokens`."
