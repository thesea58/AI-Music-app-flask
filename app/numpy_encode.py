"""
MIDI to numpy array encoding
source: https://github.com/bearpelican/musicautobot/blob/master/musicautobot/numpy_encode.py
"""
...
def midi2npenc(midi_file, skip_last_rest=True, chuyentone = False, check_part_mono = False):
    "Converts midi file to numpy encoding for language model"
    stream = file2stream(midi_file)  # 1.
    if check_part_mono:
        if len(stream.getElementsByClass(music21.stream.Part))>1:
            return 'có nhiều part trong file midi'
    if chuyentone:
        stream = chuyenTone(stream)
    chordarr = stream2chordarr(stream)  # 2.
    return chordarr2npenc(chordarr, skip_last_rest=skip_last_rest)  # 3.

def midi2text_encod(file_midi):
  npenc = midi2npenc(file_midi,chuyentone = True)
  # print(file_midi)
  text_npenc_mono = npenc2text_encod(npenc)
  # save_midi_from_text_symbol(npenc_mono,file_name='mel.mid')
  return text_npenc_mono 

# List[Timestep][NoteEnc] -> 'note _ _ _ r _ _ _ '
def npenc2text_encod(npenc):
  npenc= list(npenc)
  if npenc[-1][0] != -1:
      npenc.append([-1,npenc[-1][1]])
  # if check_mono(npenc) is False:
  #     print("file midi không mono")
  #     return 
  npenc_mono = str()
  for idx in range(len(npenc)-1):
      step = npenc[idx]
      step_Pre = npenc[idx-1]
      if step[0] != -1:
          npenc_mono += step2text(step)
      elif idx == 0:
          npenc_mono += step2text(step)
      elif step[1] > step_Pre[1]:
          npenc_mono += step2text([-1, step[1] - step_Pre[1]])
...
def stream2chordarr(s, note_size=NOTE_SIZE, sample_freq=SAMPLE_FREQ, max_note_dur=MAX_NOTE_DUR):
    ...
    def note_data(pitch, note):
        return (
            pitch.midi, int(round(note.offset * sample_freq)), int(round(note.duration.quarterLength * sample_freq)))
...
def chuyenTone(song):
    ...
    """
    song = song.getElementsByClass(music21.stream.Part)
    measures_part0 = song[0].getElementsByClass(music21.stream.Measure)
    key = None

    # Loop through all elements in the part
    for element in measures_part0:
    ...
    if not isinstance(key, music21.key.Key):
        main_stream = song.parts[0]
        key = main_stream.analyze('key')
    # print(key)
...
def save_midi_from_text_symbol(Melody, step_duration=0.25, format="midi", file_name="mel.mid"):
